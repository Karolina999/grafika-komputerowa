...
//        gl2.glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 0, 100, 100, 100, 0);
//        gl2.glEnable (GL_TEXTURE_2D);
        gl2.glBegin (GL_TRIANGLE_FAN);
        gl2.glNormal3d(0,0,1);
        gl2.glTexCoord2d(0.2,0.1);
        gl2.glVertex2f (-0.7f * size, -0.5f * size);
        gl2.glTexCoord2d(0.5,0.1);
        gl2.glVertex2f (0.7f * size, -0.5f * size);
        gl2.glTexCoord2d(0.25,0.5);
        gl2.glVertex2f (0 * size, 0.7f * size);
        gl2.glEnd ();
    }
    ...
    private Texture textureFromResource(String resourceName) throws IOException {
        ...
        URL texURL;
        texURL = this.getClass().getClassLoader().getResource(resourceName);
        BufferedImage image = ImageIO.read(Objects.requireNonNull(texURL));
        Texture texture;
    ...
    private Texture textureFromPainting() {
        ...
        GLContext ctx = displayGL.getContext();

        if(!ctx.isCurrent())
        {
            ctx.makeCurrent();
        }
        GL2 gl2 = ctx.getGL().getGL2();
    ...
    public JMenuBar getMenuBar() {
        ...
        makeMenuItem(objectMenu, "Piramid", objectListener, 7);
