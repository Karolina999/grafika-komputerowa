...
public class Lab6 extends GLJPanel implements GLEventListener  {
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        float[] zero = { 0, 0, 0, 1 };
        gl2.glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, gray, 0);
        gl2.glMaterialfv(GL_FRONT_AND_BACK, GL2.GL_SPECULAR, zero, 0);
    ...
    private void triangle(GL2 gl2, float size){
        ...
        gl2.glVertex3f (-0.7f * size, 0,-0.5f * size);
        gl2.glVertex3f (0.7f * size, 0,-0.5f * size);
        gl2.glVertex3f (0 * size, 0,0.7f * size);
        gl2.glEnd ();
    }

        private void triangle2(GL2 gl2, float size){
            gl2.glBegin(GL_TRIANGLE_FAN);
            gl2.glVertex3f(-0.7f * size, 0, 0f);
            gl2.glVertex3f(0.7f * size, 0, 0f);
    ...
    private void drawpyramid(GL2 gl2) {
        ...
        for (int i=0; i<7; i++) {
            gl2.glPushMatrix();
            gl2.glRotatef(angle * i, 0, 1, 0);
            gl2.glTranslatef(0,0,-0.75f * size);
        ...
        for (int i = 0; i < 7; i++) {
            gl2.glPushMatrix();
            gl2.glRotatef(i * angle, 0f, 1f, 0f);
            gl2.glTranslatef(0, 0f, -1.3f * size);
            gl2.glScalef(0.95f,3f,1.43f);
            gl2.glRotatef(65, 1f, 0f, 0f);

            //3 - światło - wektory normalne
            gl2.glNormal3f( (float)Math.cos(Math.toRadians(65)), (float)Math.sin(Math.toRadians(65)), 0 );
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        gl2.glLightfv (GL_LIGHT1, GL_DIFFUSE, materials[11],0);
        gl2.glLightfv (GL_LIGHT1, GL_SPECULAR, materials[11],0);
        //gl2.glLightfv (GL_LIGHT1, GL_AMBIENT, materials[11],0);

        float[] position = { 10,2,3,0 };
        gl2.glLightfv(GL_LIGHT1, GL_POSITION, position, 0);


        gl2.glEnable(GL_LIGHT2);
        gl2.glLightfv (GL_LIGHT2, GL_DIFFUSE, materials[4],0);
        //gl2.glLightfv (GL_LIGHT2, GL_SPECULAR, materials[4],0);
        gl2.glLightfv (GL_LIGHT2, GL_AMBIENT, materials[4],0);

        float[] position2 = { -10,2,1,0 };
        gl2.glLightfv(GL_LIGHT2, GL_POSITION, position2, 0);

        gl2.glEnable(GL_LIGHT3);
        //gl2.glLightfv (GL_LIGHT3, GL_DIFFUSE, materials[3],0);
        gl2.glLightfv (GL_LIGHT3, GL_SPECULAR, materials[3],0);
        gl2.glLightfv (GL_LIGHT3, GL_AMBIENT, materials[3],0);

        float[] position3 = { -10,2,30,1 };
        gl2.glLightfv(GL_LIGHT1, GL_POSITION, position3, 0);


        //5 - oświetlenie globalne
        float ambientLevel [] = {0.15f,0.15f,0.15f,1};
